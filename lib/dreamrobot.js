...
module.exports = function Service(config, emitter) {
	...
	function fetchAndSaveToken(authParameters) {
		return new Promise((resolve, reject) => {
			...
			// try {
				...
				// 	resp.on('end', () => {
				// 		authTokenResponse = JSON.parse(data);
				//
				...
				// });
				// con.write("{\"grant_type\":\"client_credentials\"}"/* + JSON.stringify(authParameters)*/);
	...
	this.makeRequest = function makeRequest(rest_path, method, body) {
		...
		auth().then(function(accessToken) {
			...
			return new Promise(function(resolve, reject) {
				request(parameters).then(function(response){
					...
					request(new_parameters);

					if (response.statusCode >= 300) {
						reject(`Request-Error: ${response.body.error}: ${response.body.error_message}`);
	...
	//					throw new Error(`Request-Error: ${response.body.error}: ${response.body.error_message}`);
				...
				//Rückgabe testweise an API als LOG-Ausgabe senden
				/*{
					const new_parameters = {
						followAllRedirects: true,
						url: buildUrl('sleep'),
