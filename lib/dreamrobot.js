...
module.exports = function Service(config, emitter) {
	...
	this.webhookStartup = async function webhookStartup(cfg, section, eventToListenTo) {
		const response = await this.makeRequest('admin/event/handler', 'POST', {
			type:{id:eventToListenTo},
			action:{
				id: 'notification',
				required: {
