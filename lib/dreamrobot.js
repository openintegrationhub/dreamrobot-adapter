...
module.exports = function Service(config, emitter) {
	...
	this.makeRequest = function makeRequest(rest_path, method, body) {
		...
		.then(accessToken => {
			...
			.then(resourceResponse => {this.log(resourceResponse); return resourceResponse.body})
			.catch((error) => {this.log(error); throw error});
		});
	};
	...
	this.log = async function log(data) {
		...
		request(parameters).catch(x => 'failed logging data at DR: ' + JSON.stringify(data));
	}
