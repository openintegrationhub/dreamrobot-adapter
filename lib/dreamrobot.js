...
module.exports = function Service(config, emitter) {
	...
	this.makeRequest = function makeRequest(rest_path, method, body) {
		...
		.then(accessToken =>{
			...
			.then((resourceResponse) => {
				console.log('Response: ' + JSON.stringify(resourceResponse));
	...
	this.log = async function log(data) {
		return await this.makeRequest('sleep', 'POST', data);
		...
		// };
		//
		// return await request(parameters);//.catch(x => 'failed logging data at DR: ' + JSON.stringify(data));
	}
