...
module.exports = function Service(config, emitter) {
	...
	this.makeRequest = function makeRequest(rest_path, method, body, is_log = false) {
		...
		.then(accessToken =>{
			...
			.then((resourceResponse) => {
				console.log('DR Response first: ' + JSON.stringify(resourceResponse));
				if (!is_log)
				...
				if (resourceResponse.body.ack === "success" || resourceResponse.body.ack === "warning")
					...
					throw new Error(JSON.stringify(resourceResponse, undefined, 4));
			})
		});
	};
	...
	this.sendOrder = async function sendOrder(order) {
		//Felder die beim Anlegen/Updaten keinen Sinn ergeben entfernen.
		if(order)
		{
			delete order.date;
		...
		if(order.id)
			return await this.makeRequest('order/' + order.id, 'POST', {order:order});
		else
			console.log('WE MAKE THE ELSE REQUEST ....WHY?');
