...
module.exports = function Service(config, emitter) {
	...
	console.log('construct Dreamrobot after buildUrl');
	...
	console.log('construct Dreamrobot after wasAuthenticationSuccessful');

	function fetchAndSaveToken(authParameters) {
		const now = new Date();

		const authTokenResponse = request({
			...
			body: authParameters,
			header: {
				'Authorization': 'Basic ' + btoa([config.username, config.password].join(':'))
			}
		});
	...
	console.log('construct Dreamrobot after fetchAndSaveToken');

	function getTokenUsingUserCredentials() {
	...
	console.log('construct Dreamrobot after getTokenUsingUserCredentials');
	...
	console.log('construct Dreamrobot after getAccessTokenUsingRefreshToken');
	...
	console.log('construct Dreamrobot after auth');
	 ...
	 */
	this.makeRequest = function makeRequest(rest_path, method, body = undefined) {
		...
		console.log(`About to make request.  Url: ${parameters.rest_path}, Method: ${method}`);

		const response = request(parameters);

		if (!wasAuthenticationSuccessful(response)) {
			return "ERROR: Authentication not Successful: " + JSON.stringify(response.body);
