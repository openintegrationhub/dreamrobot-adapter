...
module.exports = function Service(config, emitter) {
	 ...
	 */
	this.makeRequest = async function makeRequest(rest_path, method, body = undefined) {
	...
	// 	const moduleDetails
	// 		= await this.makeRequest(`metadata?type_filter=modules&module_filter=${encodeURIComponent(module)}`, 'GET');
	...
	// this.webhookStartup = async function webhookStartup(config, eventToListenTo) {
