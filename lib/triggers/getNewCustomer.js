...
module.exports.startup = async function startup(cfg) {
	...
	global.eventHandlerId = ret.event_handler_id;
};

//Aktion
module.exports.process = async function ProcessTrigger(msg) {
	this.emitData("123 EH-ID: " + global.eventHandlerId);
	console.log("123 EH-ID: " + global.eventHandlerId);
	// this.emitData(msg);
};
...
module.exports.shutdown = async function shutdown(cfg, startData) {
	const dr = new DreamRobot(cfg, this);
	return await dr.webhookShutdown(global.eventHandlerId);
};
